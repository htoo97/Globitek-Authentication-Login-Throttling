<?php
  ...
  function failed_login_by_username($username) {
    global $db;
  ...
  function reset_failed_count($username) {
    global $db;

    $users_result = failed_login_by_username($username);
  ...
  // Updates failed_logins database
  function failed_login($username) {
    global $db;

    $users_result = failed_login_by_username($username);

    // if username already exists, update entries
    if (db_num_rows($users_result) > 0) {
      // fetch one entry
      $user = db_fetch_assoc($users_result);
      $attempt = date("Y-m-d H:i:s");
      $sql = "UPDATE failed_logins SET ";
      $sql .= "count='" . db_escape($db, $user['count']) . "' + 1, ";
      $sql .= "last_attempt='" . $attempt . "' ";
      $sql .= "WHERE id='" . db_escape($db, $user['id']) . "' ";
      $sql .= "LIMIT 1; ";
      // For update_user statements, $result is just true/false
